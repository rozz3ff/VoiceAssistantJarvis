...
PORCUPINE_ACCESS_KEY = "YOUR_ACCESS_TOKEN" #свой ключ
...
HUGGINGFACE_API_URL = "https://api-inference.huggingface.co/models/meta-llama/Llama-3.3-70B-Instruct"
HUGGINGFACE_HEADERS = {"Authorization": "Bearer YOUR_ACCESS_TOKEN"}
...
def find_application_path_auto(query):
    ...
    query_translit = ""
    try:
        query_translit = translit(query_lower, 'ru', reversed=True)
        print(f"  Транслитерация -> '{query_translit}'")
    except translit_exceptions.LanguageDetectionError:
        print(f"  Не удалось определить язык для транслитерации запроса: '{query_lower}'.")
    ...
    best_match_original = fuzzy_process.extractOne(
        query_lower, app_index.keys(), score_cutoff=FUZZY_SCORE_CUTOFF_ORIGINAL
    )
    ...
    if best_match_translit and best_match_translit[1] > best_score:
        ...
        source = "транслитерация"

    # Проверяем результат поиска по оригинальному запросу
    ...
    if final_match:
        matched_name = final_match[0]
        score = final_match[1]
        found_path = app_index[matched_name]
        ...
        print(f"  Не найдено подходящего приложения для запроса '{query_lower}' в индексе.")
        return None, None
...
def correct_command_fuzzy(input_command):
    ...
    # Сначала проверяем точные совпадения с начала команды
    for cmd in known_commands:
        if input_command.startswith(cmd):
            return input_command  # Возвращаем оригинал если начало совпадает
    ...
    if best_match:
        ...
        # Если оригинальная команда длиннее найденного префикса
        if len(input_command) > len(matched_prefix):
            # Возвращаем исправленный префикс + остаток команды
